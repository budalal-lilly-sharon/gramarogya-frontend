  ...
  const getGuidance = () => {
    ...
    if (s.includes('jwaram') || s.includes('fever') || s.includes('జ్వరం')) {
      return {
        ...
        tips: te? ['ఎక్కువ నీళ్లు తాగండి', 'పూర్తిగా విశ్రాంతి తీసుకోండి', 'పారాసిటమాల్ గురించి డాక్టర్‌ను అడగండి'] : ['Drink plenty of water and stay hydrated', 'Take complete rest, avoid cold exposure', 'Ask doctor about common medicines like Paracetamol'],
    ...
    if (s.includes('headache') || s.includes('tala') || s.includes('తల') || s.includes('head')) {
      return {
        ...
        tips: te? ['చీకటి గదిలో విశ్రాంతి తీసుకోండి', 'నీళ్లు తాగండి, ఫోన్ తగ్గించండి', 'తలకు మసాజ్ చేయండి'] : ['Rest in a dark and quiet room', 'Drink water and reduce screen time', 'Light head massage can help'],
        when: te? '3 రోజుల కంటే ఎక్కువ తలనొప్పి ఉంటే డాక్టర్‌ను కలవండి.' : 'If headache persists for more than 3 days, consult doctor.',
      }
    }
    if (s.includes('cough') || s.includes('cold') || s.includes('daggu') || s.includes('జలుబు')) {
      return {
        ...
        tips: te? ['రోజుకు రెండుసార్లు ఆవిరి పట్టండి', 'తేనెతో గోరువెచ్చని నీళ్లు తాగండి', 'దుమ్ము, చల్లని పానీయాలు వద్దు'] : ['Steam inhalation twice a day', 'Drink warm water with honey', 'Avoid dust and cold drinks'],
        when: te? '7 రోజుల కంటే ఎక్కువ దగ్గు ఉంటే డాక్టర్‌ను కలవండి.' : 'If cough lasts more than 7 days, consult doctor.',
      }
    }
    return {
      title: te? `🩺 లక్షణం: "${userSymptom}"` : `🩺 Symptom: "${userSymptom}"`,
      tips: te? ['సరైన సలహా కోసం డాక్టర్‌ను సంప్రదించండి', 'సొంతంగా మందులు వాడకండి', 'మీ పూర్తి వివరాలు డాక్టర్‌కు చెప్పండి'] : ['Please consult our online doctors for accurate advice', 'Avoid self-medication', 'Share your complete history with doctor'],
    ...
    <div style={{ minHeight:'100vh', background:'#FFFEFB', maxWidth:430, margin:'0 auto', paddingBottom:90 }}>
      ...
      {userSymptom && guidance && (
        <div style={{ margin:'12px 16px 0 16px', background: isTelugu? '#fffbeb' : '#eff6ff', border:`1.5px solid ${isTelugu? '#fde68a' : '#bfdbfe'}`, borderRadius:16, padding:12 }}>
          ...
          <button onClick={()=>{localStorage.removeItem('userSymptom'); setUserSymptom(''); nav('/doctors')}} style={{ marginTop:10, fontSize:10, background:'#111827', color:'white', border:'none', padding:'6px 12px', borderRadius:100, fontWeight:800, cursor:'pointer' }}>{isTelugu? 'తొలగించు ✕' : 'Clear ✕'}</button>
        </div>
      )}
